...
var WebBook = function () {
	function WebBook(bookContainer, options) {
		...
		//div container
		this._divContainer = this._text.cloneNode(true);
		...
		value: function init() {
			...
			var promise = new Promise(function (resolve, reject) {
				if ('webkitColumnWidth' in document.body.style || 'mozColumnWidth' in document.body.style || 'columnWidth' in document.body.style) {
					...
					_this.col = true;
					//text-container
					var cs = _this._textContainer.style;
					cs.display = "none";
					...
					cs.padding = "0px";
					cs.height = _this.getHeight() + "px";
					cs.maxWidth = _this.getMaxWidth() + "px"; //maxWidth : responsive
					cs.display = "block";
					_this._containerWidth = _this._textContainer.clientWidth; //responsive
					//text clone
					var ts = _this._divContainer.style;
					var marginX = _this.getMarginX();
					var marginY = _this.getMarginY();
					...
					ts.webkitColumnWidth = _this._containerWidth + "px";
					ts.mozColumnWidth = _this._containerWidth + "px";
					ts.columnWidth = _this._containerWidth + "px";
					ts.mozColumnGap = marginX * 2 + "px";
					ts.webkitColumnGap = marginX * 2 + "px";
					ts.columnGap = marginX * 2 + "px";

					_this._textContainer.replaceChild(_this._divContainer, _this._text);
					_this._text = bookContainer.querySelector('[data-wb-text]');
					//this._text.style.display = 'block';
					setTimeout(fctInit, 50);
				} else {
			...
			var promise = new Promise(function (resolve, reject) {
				if ('webkitColumnWidth' in document.body.style || 'mozColumnWidth' in document.body.style || 'columnWidth' in document.body.style) {
					...
					var _loop2 = function _loop2(_i2) {
						var id = _this2._elPageNumbers[_i2].getAttribute('data-wb-element-page-number');
						var pageNumber = _this2._sections_page_start.filter(function (o) {
							return o.id === id;
						})[0].page_start;

						if (pageNumber < 1) {
							_this2._elPageNumbers[_i2].innerHTML = "";
						} else if (_this2._elPageNumbers[_i2].innerHTML != pageNumber) {
							_this2._elPageNumbers[_i2].innerHTML = pageNumber;
						}
					};

					for (var _i2 = 0; _i2 < _this2._elPageNumbers.length; _i2++) {
						_loop2(_i2);
					}
		...
		value: function refresh() {
			...
			if (this.col === false) {
				...
				for (var _i8 = 0; _i8 < this._currentPages.length; _i8++) {
					...
					} else if (this._currentPages[_i8].innerHTML !== _pageNumber) {
						this._currentPages[_i8].innerHTML = _pageNumber;
					}
				}
				...
				for (var _i9 = 0; _i9 < this._currentTotalPages.length; _i9++) {
					...
					} else if (this._currentTotalPages[_i9].innerHTML !== _pageNumber2 + "/" + this.pages_total) {
						this._currentTotalPages[_i9].innerHTML = _pageNumber2 + "/" + this.pages_total;
					}
				}
